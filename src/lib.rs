...
impl RangeMin<Range<usize>> for RangeMinLinear{
    fn range_min(&self, range: Range<usize>) -> usize{
        ...
        let middle = chunk_id * chunk_size + self.lookup_table[self.chunk_content[chunk_id]] as usize;

        *[s, middle, e].into_iter().min_by_key(|&&x| self.data[x]).unwrap()
    }
}
...
mod tests{
    ...
    fn test_build_chunk_table(){
        ...
        assert_eq!(RangeMinLinear::build_chunk_table(data, chunk_size), expected);
    ...
    fn test_build_lookup(){
        ...
        for i in 0..512{
            ...
            let (min_id, _min) = (0..16)
                ...
                .unwrap();
            assert_eq!(min_id, table[i] as usize, "error on value = 0b{:b}", i);
        }
    }
